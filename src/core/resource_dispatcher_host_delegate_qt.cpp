...
void ResourceDispatcherHostLoginDelegateQt::triggerDialog()
{
    ...
    content::RenderFrameHost* frameHost = content::RenderFrameHost::FromID(m_renderProcessId, m_renderFrameId);

    if(!frameHost)
       return;

    content::RenderViewHost* renderViewHost = frameHost->GetRenderViewHost();
